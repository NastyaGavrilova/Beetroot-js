...
ShopCheck.prototype.toString = function () {
  return `${this.nameOfProduct} (x${this.amount}), ${this.price}`;
};

// function showCheckList(spanEl) {
//   let str = " ";
//   for (let i = 0; i < shopCheck.length; i++) {
//     if (shopCheck[i] !== undefined) str += i + " - " + shopCheck[i] + "<br>";
//   }
//   return (spanEl.innerHTML = str);
// }

let showCheckList = (spanEl) => {
  let str = `${this.nameOfProduct} (x${this.amount}), ${this.price}`;
  for (let i = 0; i < shopCheck.length; i++) {
    if (shopCheck[i] !== undefined) str += i + " - " + shopCheck[i] + "<br>";
  }
  return (spanEl.innerHTML = str);
