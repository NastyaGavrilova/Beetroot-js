...
function showTime(obj) {
  ...
  return (timeInformation.innerHTML = `«${obj.hour.value}:${obj.minute.value}:${obj.sec.value}»`);
}

function addSeconds(obj1) {
  let addSecInfo = document.getElementById("js-task3-span2");
  if (obj1.sec.value <= 0) {
    return (addSecInfo.innerHTML = "Вы ввели не корректное время секунд");
  } else if (obj1.sec.value >= 60) {
    let differenceMin = Math.floor(obj1.sec.value / 60);
    obj1.sec.value = obj1.sec.value % 60;
    return;
  } else {
