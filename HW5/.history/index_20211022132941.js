...
function bigPriceInChek(arr) {
  //функция получения самого дорого товара в чеке
  let result = 0;
  const initialValue = 0;
  result = arr.reduce(ShopCheck.sortExpensive {
    return Math.max(result, b.amount * b.price);
  }, initialValue);
  return result;
}
...
//   for (let i = 0; i < shopCheck.length; i++) {
//     if (
//       shopCheck[i].amount * shopCheck.price >
...
btn2_3.addEventListener("click", (e) => {
  ...
  return (outputExpensiveProd.innerHTML = bigPriceInChek(shopCheck));
});
