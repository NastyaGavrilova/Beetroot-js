...
// !9th task in index.html

// 10th task

let userNum = +prompt("Угадайте число от 0-100");
let low = 0;
let high = 100;
let middle = Math.floor(low+high)
...
while (true) {
  ...
  let answer = prompt(
    "Введите- h, чтобы указать, является ли предположение слишком высоким. Введите - l, чтобы указать, если предположение слишком низкое. Введите - c, если я правильно угадала."
  );
  if (answer == "h") {
    high = ans - 1;
  } else if (answer == "l") {
    low = ans + 1;
  }else if(answer == "c" && answer == userNum)
}
