...
].map((prodDef) => new audincesList(prodDef));


let showaudiences = (spanEl) => {
  let str = " ";
  for (let i = 0; i < audincesList.length; i++) {
    if (audincesList[i] !== undefined)
      str += i + " - " + audincesList[i].toString() + "<br>";
  }
  return (spanEl.innerHTML = str);
};
