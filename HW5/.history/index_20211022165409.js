...
let showAudiencesByFaculty = (arr, faculty) => {
  // при вводе существующего продукта его статус меняеться на "куплено"
  // let item = shopList.find((item) => item.faculty === faculty.faculty);
  // return item.faculty;
};
...
btn4_1.addEventListener("click", (e) => {
  ...
  showAudiencesByFaculty({ faculty: facultyName.value });
  showAudiences(audincesList, audiencesShow1);
});
