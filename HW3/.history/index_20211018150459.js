...
btn5.addEventListener("click", (e) => {
  ...
  let sum = task5.value - 1;
  let temp = task5.value;
  let span5 = document.getElementById("js-task5-span");

  for (let i = 2; task5.value > 1; ) {
    if (!(task5.value % i)) {
      sum -= i;
      task5.value = task5.value / i;
    } else i++;
  }
  if (!sum) {
    return (span5.innerHTML = `Число ${temp} совершенное!`);
  } else {
    return (span5.innerHTML = `Число ${temp} не совершенное!`);
  }
});
