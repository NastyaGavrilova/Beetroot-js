...
btn5.addEventListener("click", (e) => {
  ...
  let temp = 0;
  for (let i = 1; i <= task5.value / 2; i++) {
    if (task5.value % i === 0) {
      temp += 1;
    } else break;
  }
  if (temp === task5.value && temp !== 0) {
    return (span5.innerHTML = `Число ${task5.value} совершенное число!`);
  } else {
    return (span5.innerHTML = `Число ${task5.value} не совершенное число!`);
  }
});
